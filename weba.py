...
# Threading control for continuous beep
...
def play_single_beep():
    try:
        ...
        sound.play()
        time.sleep(4)  # Adjusted to match the 4-second beep duration
...
def detect_drowsiness_stream(video_source=0):
    ...
    stframe = st.empty()
    status_text = st.empty()  # For showing current status
        ...
        for result in results:
            for box in result.boxes:
                ...
                if model.names[class_id] == "Drowsy":
                    ...
                    # Only consider drowsy if eyes closed for more than 3 seconds
...
# st.write("""
# ### How it works:
# - Eyes must be closed continuously for 3 seconds to trigger drowsiness alert
# - Green box: Eyes closed but under threshold
# - Red box: Drowsy state detected
# - Status bar shows current eye closure duration
# """)
